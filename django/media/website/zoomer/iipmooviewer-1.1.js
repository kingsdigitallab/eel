/*
 * WARNING: modified version. This version avoid some of the situations where the thumbnail is too large.
 * See changes in calculateMinSizes()
 */
/*
...
var IIP = new Class({
  ...
  calculateMinSizes: function(){
	  ...
	  // GN: 06/07/11
	  // minor changes to the dynamic size of the thumbnail (min_x, min_y)
	  // Default algorithm:
	  // * tx, ty initialised with the dimensions of the full size image
	  // * then divided by 2 until the width is inferior to thumb
	  // * thumb is a quarter of the viewport width
	  // * BUT the algorithm won't divide by 2 more than this.res times
	  // * so, somtimes the thumbnail can still be as wide as half the viewport
	  //
	  // Changes:
	  // * changed the algorithm so it ignores this.res 
	  // * it will aim at a quarter of the viewport width
	  // * but may sometimes be slightly wider.
	  // (* wider width is favoured when it is closer to the quarter than 
	  // half of that width, is that clear?!?) 

    var tx = this.max_width;
    var ty = this.max_height;
    var thumb = 100;
    var proportionx = 0.25;
    ...
    if( winWidth>winHeight ){
      // For panoramic images, use a large navigation window
      if( tx > 2*ty ) thumb = winWidth * proportionx;
      else thumb = winWidth * proportionx;
    }
    else thumb = winHeight / 4;
    // added this at a later stage as a quick fix to prevent
    // the thumnbail to be too large on a wide panoramic format. 
    if (thumb > 220) thumb = 220;

    var r = this.res;
    var distx = tx - thumb;
    var distxNew = distx;
    while( true ) {
		distxNew = parseInt(tx / 2) - thumb;
    	if (distx < Math.abs(distxNew)) break;
		tx = parseInt(tx / 2);
		ty = parseInt(ty / 2);
		// Make sure we don't set our navigation image too small!
		//if( --r == 1 ) break;
  ...
  createWindows: function(){
    ...
    new Element('img', {src: '/media/website/zoomer/djatoka.32x32.png', id: 'info', styles: { opacity: 0.8 } } ).injectInside('logo');

    // Fix IE7 PNG transparency problem
    if( Browser.Engine.trident5 ){
      $('info').setStyle( 'filter', 'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src="/media/website/zoomer/djatoka.32x32.png",sizingMethod=scale)' );
    }
  ...
  createNavigationWindow: function() {
   ...
   var loadBarContainer = new Element('div', {
	...
	id: 'navbuttons',
	html: '<img id="shiftLeft" src="/media/website/zoomer/left.png"/><img id="shiftUp" src="/media/website/zoomer/up.png"/><img id="shiftRight" src="/media/website/zoomer/right.png"/><br/><img id="shiftDown" src="/media/website/zoomer/down.png"/><br/><img id="zoomIn" src="/media/website/zoomer/zoomIn.png"/><img id="zoomOut" src="/media/website/zoomer/zoomOut.png"/><img id="reset" src="/media/website/zoomer/reset.png"/>&nbsp;&nbsp;<img id="snapshot" width="24" height="24" src="/media/website/zoomer/djatoka.32x32.png"/>'
    });
  ...
  load : function() {
    new Request( {
        ...
        onComplete : function(transport) {
            ...
            this.res = this.num_resolutions;
            this.createWindows();
            // GN: 24/10/11: added on_loaded event
