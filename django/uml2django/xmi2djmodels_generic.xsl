...
<xsl:for-each select="//UML:TaggedValue[UML:TaggedValue.type/UML:TagDefinition/@xmi.idref = $tag_base_classid]">
# Used as a base class
import <xsl:call-template name="getModulePath"><xsl:with-param name="text" select="UML:TaggedValue.dataValue/text()"/></xsl:call-template>
</xsl:for-each>
<xsl:for-each select="//UML:TaggedValue[UML:TaggedValue.type/UML:TagDefinition/@xmi.idref = $tag_modelid]/UML:TaggedValue.dataValue/text()">
# Referenced by a foreign key <!-- TODO: remove duplicates -->
import <xsl:call-template name="getModulePath"><xsl:with-param name="text" select="."/></xsl:call-template>
</xsl:for-each>
...
<xsl:call-template name="ConvertClassAssociations"><xsl:with-param name="classid" select="@xmi.id"/></xsl:call-template>
	...
	table_group = '<xsl:choose>
		...
		<xsl:otherwise>
			<xsl:variable name="class-ste"><xsl:value-of select="key('stereotypes', UML:ModelElement.stereotype/UML:Stereotype[1]/@xmi.idref)/@name" /></xsl:variable>
			<xsl:if test="string-length($class-ste) > 0"><xsl:value-of select="$class-ste"/></xsl:if>
		</xsl:otherwise>
	</xsl:choose>'
	...
	<xsl:for-each select="../UML:AssociationEnd[@xmi.id != current()/@xmi.id]" >
		...
		<xsl:choose>
	 	...
	 	<xsl:text>(</xsl:text>
	 		...
	 		<xsl:if test="descendant::UML:MultiplicityRange/@lower = '1'">blank=False, null=False, default=<xsl:call-template name="getDefaultValue"><xsl:with-param name="association" select="../.."/></xsl:call-template>, </xsl:if>
...
<xsl:template match="UML:Classifier.feature/UML:Attribute">
	...
	<xsl:choose>
		<xsl:when test="count($datatype) = 1 and $datatype/@name = 'GenericPtr'">
			<xsl:call-template name="getGenericForeignKey"><xsl:with-param name="fieldName" select="$fieldName"/></xsl:call-template>
		</xsl:when>
		<xsl:otherwise>
			...
			<xsl:if test="count($datatype) > 0">
				<xsl:for-each select="$datatype">
					<xsl:choose>
						...
						<xsl:when test="@name='Char 1024'" >CharField(max_length=1024, null=False, default="", blank=<xsl:value-of select="$optional" />, </xsl:when>
						<xsl:when test="@name='ModelPtr'" >ForeignKey(<xsl:value-of select="$attribute//UML:TaggedValue[UML:TaggedValue.type/UML:TagDefinition/@xmi.idref = $tag_modelid]/UML:TaggedValue.dataValue/text()" />, </xsl:when>
						<xsl:otherwise>
							...
							<xsl:if test="@name = 'File'">upload_to='<xsl:value-of select="$attribute/UML:ModelElement.taggedValue/UML:TaggedValue[key('tags', UML:TaggedValue.type/UML:TagDefinition/@xmi.idref)/@name = 'Upload Path']/UML:TaggedValue.dataValue/text()" />', </xsl:if>
							...
							<xsl:if test="@name = 'Slug'">max_length=250, </xsl:if>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:for-each>
			</xsl:if>
