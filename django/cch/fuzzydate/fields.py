...
from cch.fuzzydate.core import FuzzyDate, modifiers
from cch.fuzzydate import forms
...
def _precision_field_name(name): return "%s_precision" % name


def _date_to_field_name(name): return "%s_to" % name


def _date_mod_field_name(name): return "%s_mod" % name
...
class FuzzyDateCreator(object):
    ...
    def __get__(self, obj, type=None):
        ...
        if date is None:
            ...
            return FuzzyDate(date, getattr(obj, self.date_to_name),
                             modifiers.getElement(modifier))
...
class FuzzyDateField(models.DateField):
    ...
    def __init__(self, *args, **kwargs):
        if ('modifier' in kwargs):
            self.useModifier = (kwargs['modifier'] is True)
            # remove it from the kwargs otherwise the DateField will scream in
            # horror.
            del kwargs['modifier']
    ...
    def contribute_to_class(self, cls, name):
        ...
        if (self.useModifier):
            ...
            # models _meta.fields - to achieve this, we need to change it's
            # creation_counter class variable.
            date_modifier = models.SmallIntegerField(
                null=False, default=0, blank=False, editable=False)
            # setting the counter to the same value as the date field itself will
            # ensure the precision field appear first - it is added first after all,
            # and when the date field is added later, it won't be sorted before
            # it.
    ...
    def get_db_prep_lookup(self, lookup_type, value):
        if lookup_type == 'exact':
            ...
            # like 'range'.
            return super(FuzzyDateField, self).get_db_prep_lookup(
                lookup_type, value)
    ...
    # GN: This is for deprecated South database migration system.
    # http://south.readthedocs.io/en/latest/customfields.html/
    # TODO: dango 1.7: use django migration system.
    # https://stackoverflow.com/questions/29551367
    @classmethod
    def declare_field_to_database_migration_engine(cls):
        import re
        ...
        rules = [
            (
                ...
                [],
                {
                    'modifier': ['useModifier', {'default': False}],
                },
            )
        ]
        add_introspection_rules(rules, ['^%s' % re.escape(path)])


FuzzyDateField.declare_field_to_database_migration_engine()
