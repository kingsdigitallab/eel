# $Id: ldap_backend.py 620 2010-08-05 10:55:36Z gnoel $
...
class LDAPBackend(object):
    ...
    def _update_user(self, l, user):
        ...
        # GN: all users are staff
        if self.settings['LDAP_ALL_STAFF']: user.is_staff = True
        if self.settings['LDAP_DEFAULT_GROUP'] is not None:
            try:
                user.save()
                from django.contrib.auth.models import Group
                user.groups.add(Group.objects.get(name=self.settings['LDAP_DEFAULT_GROUP']))
            except:
                pass
